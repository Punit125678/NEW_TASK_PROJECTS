...
function FINAL_PAYMENT(paymentMode) {
    LAST_PAYMENT_DETAILS.cardExp = document.getElementById("CARD_EXP").value;
...
function test(){
    date = new Date();
    console.log(date);
    date.setMilliseconds(date.getMilliseconds()+2419200000);
...
function SHOW_SUMMARY(paymentMode) {
    ...
    let date = new Date();
    let expMonthBox = document.getElementById("S_EXP_MONTH");
    let expDateBox = document.getElementById("S_EXP_DATE");
    ...
    expMonthBox.innerHTML = test();
    expDateBox.innerHTML = "N/A";
    ...
    if (paymentMode === "UPI Payment") {
        ...
        let cardExp = LAST_PAYMENT_DETAILS.cardExp || document.getElementById("CARD_EXP").value;
        let last4 = cardNo.slice(-4);
        let expParts = cardExp.split("-");
        let expYear = expParts[0] || "";
        let expMonth = expParts[1] || "";

        expMonthBox.innerHTML = expMonth || "N/A";
        expDateBox.innerHTML = expYear || "N/A";

        extraBox.innerHTML =
            "<b>Card Details</b><br>" +
            "Card Number :- XXXX-XXXX-XXXX-" + last4 + "<br>" +
            "Card Expiry :- " + (expMonth && expYear ? expMonth + "/" + expYear : "N/A");
    }
